...
echo character list: abcdefghijklmnopqrstuvwxyz_
echo keep the key private

:start

set /p string=string to encrypt : 
set /p key=encryption key (number between 0 and 27) : 
set /p mod=encryption key modifier (big number) : 
...
echo close the window now to avid writing the result into result.txt
pause
